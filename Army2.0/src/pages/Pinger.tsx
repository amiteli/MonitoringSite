...
export function stringToDateTimeObject(dateTimeString: string): Date
{
  ...
  let date = new Date();
  date.setDate(parseInt(dateTimeString.substring(0, dateTimeString.indexOf("/"))));
  dateTimeString = dateTimeString.substring(dateTimeString.indexOf("/") + 1);
  date.setMonth(parseInt(dateTimeString.substring(0, dateTimeString.indexOf("/"))));
  dateTimeString = dateTimeString.substring(dateTimeString.indexOf("/") + 1);
  date.setFullYear(parseInt(dateTimeString.substring(0, dateTimeString.indexOf(" "))));
  dateTimeString = dateTimeString.substring(dateTimeString.indexOf(" ") + 1);
  date.setHours(parseInt(dateTimeString.substring(0, dateTimeString.indexOf(":"))));
  dateTimeString = dateTimeString.substring(dateTimeString.indexOf(":") + 1);
  date.setMinutes(parseInt(dateTimeString.substring(0, dateTimeString.indexOf(":"))));
  dateTimeString = dateTimeString.substring(dateTimeString.indexOf(":") + 1);
...
const Pinger = (props: PingerProps) => 
{
    ...
    const handleStartChange = (newStart: Date | null) => {
        if (newStart && endDate && newStart < endDate && newStart >= defaultStartTime) {
    ...
    {
        let chartParent = (document.getElementsByClassName("flex-timeline-chart-container")[0] as HTMLDivElement);
        let parentWidth = chartParent.clientWidth;
        let chartContainerWidth = 0;
        for (let i = 0; i < chartParent.children.length; i++) {
            chartContainerWidth += (chartParent.children[i] as HTMLDivElement).clientWidth;
        }
    ...
    const fetchPingHistory = async (): Promise<StateRecord> => 
    {
        ...
        if (!res.ok) {
            ...
            setErrorText(`status code: ${res.status} status text: ${res.statusText} url: ${res.url}`);
            throw new Error("Problem fetching data");
        }
        return res.json();
    };

    const { data, isLoading, isError } = useQuery<StateRecord>("history", fetchPingHistory,
    {
        onSuccess: (data) =>
        {
            if (useCustomizedEndDate == false)
            {
                setDatabase(data);
                setEndDate(new Date());
                console.log("Data is: " + data + " End is: " + endDate?.toString() + ", Customized: " + useCustomizedEndDate);
            }
        }
    });
    ...
    {        
        ...
        function zoomOutClick(event: React.MouseEvent<HTMLDivElement, MouseEvent>): void {
            if (zoomLevel > 1) {
                setZoomLevel(zoomLevel - 1);
            }
        }
    
        function zoomInClick(event: React.MouseEvent<HTMLDivElement, MouseEvent>): void {
            setZoomLevel(zoomLevel + 1);
        }
            ...
            if (unit == selectedUnit)
            {
                ...
                Object.keys(database[unit]).map((category: string) => 
                {
                    Object.keys(database[unit][category]).map((ip: string) =>
                    {
                        ...
                        {
                            selectedStart: records[recordIndex] ? records[recordIndex].selectedStart : null,
                            selectedEnd: records[recordIndex] ? records[recordIndex].selectedEnd : null,
                            ipAddress: ip,
                            statesProps: [],
                            secondsPer180Px: 0,
                            totalWidth: 0,
                            upTime: 0,
                            downTime: 0,
                            hidden: records[recordIndex] ? records[recordIndex].hidden : false,
                            clickedIp: clickedIp
                        };
                        ...
                        database[unit][category][ip].map((state: State) => 
                        {
                            ...
                            let endStateTime = database[unit][category][ip].length > index + 1 ? database[unit][category][ip][index + 1].time : currentDateTime;
                            let startDateTime = stringToDateTimeObject(state.time);
                            ...
                            if (inTime == true)
                            {
                                let stateDurationSeconds = (endDateTime.getTime() - startDateTime.getTime()) / 1000;
                                
                                if (state.state == "up") timelineRecord.upTime += stateDurationSeconds;
                                if (state.state == "down") timelineRecord.downTime += stateDurationSeconds;
                        
                                let timelineChartItem: TimelineChartItemProps =
                                {
                                    color: state.state == "up" ? "#29B971" : (state.state == "down" ? "#FF5555" : "#757575"),
                                    state: state.state,
                                    start:
                                    {
        ...
        <div>
            <div className='filters-wrapper'>
                    ...
                    <CacheProvider value={cacheRtl}>
                        <ThemeProvider theme={theme}>
                            <LocalizationProvider dateAdapter={AdapterDateFns}>
                                <div className="datepicker-label">הצג נתונים מ (תאריך/שעה)</div>
                ...
                <div className='filter-dt-w' style={{marginRight: "25px"}}>
                    <CacheProvider value={cacheRtl}>
                        <ThemeProvider theme={theme}>
                            <LocalizationProvider dateAdapter={AdapterDateFns}>
                                <div className="datepicker-label">הצג נתונים עד (תאריך/שעה)</div>
                ...
                <div className='pinger-header-button noselect' style={{marginRight: "25px"}} 
                    onClick={(event) => setRealtime()}><span className="ph-span">איפוס זמן סיום</span></div>   

                <div className='pinger-header-button noselect'
                    onClick={(event) => setStartDate(defaultStartTime)}><span className="ph-span">איפוס זמן התחלה</span></div>

                <div className='pinger-header-button noselect' style={{minHeight: "40px", minWidth: "40px", borderRadius: "50%", height: "40px", width: "40px"}}
            ...
            <TimelineChart records={records} setRecords={setRecords} startDate={startDate} endDate={endDate} setStartDate={setStartDate} setEndDate={setEndDate} setCustomizedEndDate={setCustomizedEndDate} zoomLevel={zoomLevel} resolution={resolution} chartWidth={chartWidth} clickedIp={clickedIp!} />
                ...
                Object.keys(database).map((unit: string) =>
                {
                    if (unit == selectedUnit)
                    {
                        return Object.keys(database[unit]).map((category: string) => {
                            return <CategoryBlock category={category} ipBlocks={database[unit][category]} setCurrentPage={handlePageChange}/>
                        });                        
                    }
                })
            }
    ...
    return (
        <Grid container direction="column" sx={{ width: "100%" }} style={{marginTop: "-16px", paddingTop: "0px", height: "auto", maxHeight: "calc(100% - 200px)", overflowY: "auto"}}>
            <Grid container>
                <Grid item mb={2} xs={11.8} bgcolor={"rgba(242, 242, 242)"} borderRadius={4} height={"100%"}
                    maxWidth={"500px"} width={"500px"} sx={{margin: 0, padding: 0, overflowY: "hidden", overflowX: "hidden", background: "transparent"}}>
                    {result}
                </Grid>
            </Grid>
        </Grid>
